...
+ (instancetype)contactWithRancomName {
    ...
    NSString *firstName = allFirstNames[arc4random_uniform([allFirstNames count])];
    NSString *lastName = allLastNames[arc4random_uniform([allLastNames count])];
    result.name = [NSString stringWithFormat:@"%@ %@", firstName, lastName];
    result.nickName = firstName;
    result.email = [[NSString stringWithFormat:@"%@%@@email.com", firstName, lastName] lowercaseString];
    result.city = @"whereever";
    return result;
}

- (BOOL)validateEmail:(NSString **)emailValue error:(NSError * __autoreleasing *)outError {
    id value = *emailValue;
    if (value == nil) {
        value = @"";
        return YES;
    } else if (![value hasSuffix:@"@email.com"]) {
        NSLog(@"email must has suffix: @email.com");
        return NO;
    }
    return YES;
}
